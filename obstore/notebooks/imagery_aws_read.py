"""Read NZ public datasets from AWS S3 using obstore.
...
def download_dataset_images(
    ...
    """Convenience function to download all images from a NZ dataset.
    ...
    """
    if dataset_name not in NZ_DATASETS:
        raise ValueError(
            f"Unknown dataset: {dataset_name}. Choose from: {list(NZ_DATASETS.keys())}"
        )

    dataset = NZ_DATASETS[dataset_name]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Access NZ public AWS datasets (imagery/elevation/coastal) via obstore."
    )
    parser.add_argument(
        "--dataset",
...
def main() -> int:
    ...
    try:
        ...
        print(f"Error accessing NZ data: {exc}")
        return 1
