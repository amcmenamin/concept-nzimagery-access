...
LINZ_DATASETS: dict[str, DatasetInfo] = {
	"imagery": DatasetInfo(bucket="nz-imagery", region="ap-southeast-2", name="NZ Imagery"),
	"elevation": DatasetInfo(bucket="nz-elevation", region="ap-southeast-2", name="NZ Elevation"),
	"coastal": DatasetInfo(bucket="nz-coastal", region="ap-southeast-2", name="NZ Coastal"),
}
...
def check_catalog_exists(bucket: str, region: str, catalog_name: str = "catalog.json") -> bool:
...
def main() -> int:
	...
	if args.dataset:
		if args.dataset not in LINZ_DATASETS:
			print(f"Error: Unknown dataset '{args.dataset}'. Available: {list(LINZ_DATASETS.keys())}")
			return 1
