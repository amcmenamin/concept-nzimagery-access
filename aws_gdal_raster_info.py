...
def run_gdal_raster_info_cli(
	...
	*,
	...
	try:
		...
		print("Error: gdal command not found. Make sure GDAL 3.11+ is installed and in PATH.")
		return {} if format_type.lower() == "json" else ""


def run_gdal_raster_info_python(url: str, region: str = "ap-southeast-2") -> dict[str, Any]:
	...
	try:
		...
		for i in range(1, dataset.RasterCount + 1):
			...
			band_info = {
				...
				"colorInterpretation": gdal.GetColorInterpretationName(band.GetColorInterpretation()),
...
def check_file_is_cog(url: str, region: str = "ap-southeast-2") -> dict[str, Any]:
	...
	try:
		...
		result["is_tiled"] = block_x < dataset.RasterXSize or block_y < dataset.RasterYSize
...
def analyze_multiple_files(
	...
	**kwargs
	...
	# List raster files
	all_paths = list_objects(store, prefix, limit=limit * 2 if limit > 0 else 0)  # Get extra to filter
	raster_paths = [
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		"--min-max",
		action="store_true",
		help="Compute minimum and maximum values."
	)
	parser.add_argument(
		"--stats",
		action="store_true",
		help="Compute complete statistics."
	)
	parser.add_argument(
		"--approx-stats",
		action="store_true",
		help="Compute approximate statistics."
	)
	parser.add_argument(
		"--hist",
		action="store_true",
...
def main() -> int:
	...
	try:
		if args.list_prefix:
			...
			if not HAS_OBSTORE:
				print("Error: obstore required for listing. Install with: pip install obstore")
				return 1
